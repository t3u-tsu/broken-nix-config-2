...
設定一式: `nixos/installer/` / 調査メモ: `docs/research-notes.md`
...
- **対策**: カスタム ISO に以下を組み込んで、SSH だけでインストールを完結させる。
  ...
  2. 静的 IP 設定（ConoHa の割当 IP。未確定のため `wan-ip.nix` で変数化）
...
ビルド方法は 2 通り（ルート flake.nix は変更しない）:

```bash
# A. サブ flake を使う（本リポジトリの推奨。IP は wan-ip.nix で設定）
nix build path:./nixos/installer#default -o result-iso

# B. nixos-rebuild build-image（NixOS マシン上。NIX_PATH に nixpkgs が必要）
nixos-rebuild build-image --image-variant iso-installer -I nixos-config=./nixos/installer/default.nix
```

ルート flake.nix に統合する場合（別タスク）は packages に公開する:
...
`nixos/installer/wan-ip.nix` の `ipv4` / `gateway` を実 IP に置き換えてからビルドする。
...
スクリプトの内容（`nixos/installer/install-nixos.sh`）:
...
- **完全オフラインで確実に**したい場合は、ターゲットシステムのクロージャを ISO の
  ...
  ```nix
  # nixos/installer/default.nix に追加する例（ターゲット設定ができた後）
