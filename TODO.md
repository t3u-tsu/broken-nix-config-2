...
このリストは、NixOS 設定リポジトリの保守性・拡張性、および複数拠点インフラの可用性を向上させるためのタスク一覧です。

- [ ] **SOPS 機密情報の構造化と権限分離**: 現在 `secrets/secrets.yaml` に集中している機密情報を、ホスト別・サービス別に分割し、各ホストの復号権限を最小化（リスクの隔離）する。
- [ ] **flake-parts ＆ Haumea の導入とモジュール完全分離**: 既存の `modules/` をシステム（`nixos/`）とドットファイル（`home/`）に完全分離し、Haumeaで `default.nix` の手動管理を撲滅する。Void Linux等へのポータビリティもここで確保。
- [ ] **デフォルトアプリの設定調整**: XDG Mime Apps を調整し、ブラウザでファイルを開く場面でファイルマネージャ（Thunar）が開かれなかったり、ファイルマネージャ側での展開ができなかったりする不具合を解消する。
- [ ] **ログイン画面の調整**: 現在の greetd から、より軽量・モダンな `fairyglade/ly` などへの移行をテスト・調整する。
- [ ] **noctalia-shell の v5 移行と見た目調整**: noctalia-shell を v5 へ移行し、それに伴うテーマ定義の仕様変更への追従と Wayland GUI の外観・レイアウトの再微調整を行う。
