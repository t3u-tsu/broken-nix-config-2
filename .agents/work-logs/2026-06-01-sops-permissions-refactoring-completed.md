...
- **Ghostty 移行によるリモート SSH 接続時の terminfo エラー**: 手元のターミナルを Ghostty に移行した影響で、リモートサーバー（`sando-kun` など）へ SSH 接続した際に `can't find terminal definition for xterm-ghostty` という警告が発生し、`htop` などの ncurses アプリがクラッシュする問題が発生していた。
...
### 7. 全ホストへの ghostty.terminfo インストールによる SSH 端末定義エラーの解消
- 共通システムパッケージ設定 `modules/packages/base.nix` の `environment.systemPackages` に `ghostty.terminfo` を追加。
- これにより、全ホスト環境に Ghostty の terminfo 定義がインストールされ、手元の Ghostty ターミナルから各サーバーへ SSH 接続した際に発生していた `xterm-ghostty` 端末定義不在による警告や ncurses アプリのクラッシュを根本的かつ宣言的に解決した。

## 検証と適用
1. `nix flake check` を実行し、全ホスト設定で構文と依存関係の検証をパス。
2. `sudo nixos-rebuild switch --flake .#BrokenPC` を実行し、実機への適用を完全にパス。
3. **日常用 secrets 復号テスト（BrokenPC実機）**:
...
4. **段階的カナリアデプロイおよび再起動テスト（sando-kun実機）**:
   - リモートサーバー `sando-kun` (`10.0.0.2`) に対して `nixos-rebuild switch` を直接適用し、成功。
   - `sando-kun` を再起動させ、再起動後に正常に SSH 疎通が自動復旧することを確認。本設定が遠隔ホストの起動やネットワーク設定を妨げないことを確認した。
   - 起動後の `sando-kun` にて日常ユーザー `t3u` では `signing.yaml` が復号できず（日常鍵が sando-kun に配置されていないため）、管理者権限（ホスト鍵）でのみ `sando-kun.yaml` を正常に復号できるという完璧な隔離モデルが期待通り機能することを確認。
   - さらに、SSH 接続時および接続後の `htop` 起動テストで、`xterm-ghostty` に関連する端末定義警告が完全に解消されていることを実機で検証・確認した。
